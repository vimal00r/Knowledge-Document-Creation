...
def extract_flowchart_images_from_pdf(pdf_file_path, min_width=100, min_height=100):
    ...
    for page_index in range(len(pdf_file)):
        ...
        for image_index, img in enumerate(image_list, start=1):
            ...
            if image.width >= min_width and image.height >= min_height:
                ...
                save_path = os.path.join(output_dir, f"fig {n}.{output_format}")
                n += 1
    ...
    for image_path in file_list:
        ...
        inputs = processor(text=["image", "table"], images=image, return_tensors="pt", padding=True)
...
def flowchart_to_text(file_path):
    ...
    for page_num in range(len(pdf_file)):
        ...
        for c, image_info in enumerate(images, start=1):
            ...
            # image_name = f"Fig {nu}.{output_format}"
            # n += 1
            ...
            if images_in_page:
                ...
                for image in images_in_page:
                    ...
                    pdf_redact(pdf_file, page_num, fitz.Rect(image_bbox), f"fig {nu}" )
                    image_name = f"fig {nu}.{output_format}"
...
# flowchart_to_text('table_to_text_output.pdf')
